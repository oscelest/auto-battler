...
import {ComboPointActionCreateValidator, ComboPointActionPaginationValidator, ComboPointActionUpdateValidator} from "../../validators/Action/ComboPointAction.validator";

@Resolver(() => ComboPointActionEntity)
export class ComboPointActionResolver {
  
  @Query(() => ComboPointActionEntity, {nullable: true})
  public async getComboPointAction(@Arg("id") id: string, @Ctx() ctx: GraphQLContext, @Info() info: GraphQLResolveInfo) {
    const {fields, populate} = ComboPointActionEntity.fromContextToFieldsAndPopulate(info);
    
    return await ctx.entity_manager.findOne(ComboPointActionEntity, {id}, {fields, populate});
  }
